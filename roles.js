...
const Roles = (() => {
	...
	/* Definition of all tokens present in the game
		TokenID:
			...
			* disable: if present and true, excludes the token from consideration in the UI and logic. Automatically set at initialization if the prerequisites cannot be satisfied (e.g. required role is diabled)
	*/
	const TOKENS = {
		...
		MARK_VAMPIRE:			{ type: "mark", icon: { x: 0, y: 3 }, usedBy: [ ], prereq: { type: "team", any: ["TEAM_VAMPIRE"] } },	//usedBy populated at init with all vampire team members
	...
	*/
	...
	//Injects tags into the role definitions
	function _applyTags() {
	...
	//Injects teams into the role definitions
	...
	//Injects active phase into the role definitions
	...
	//Injects generic data into the role/token definitions and performs a sanity check, logging potential errors
	function _finalizeData() {
		...
		//Finalize tokens
		//Add vampire team to the vampire mark
	...
	//Locks the role/token definitions as immutable
	...
	//Takes a role definition object or a role ID and returns the corresponding role definition object
	function _getRole(role) {
		return typeof role === "object" ? role : ROLES[role];
	}
	//Takes a token definition object or a token ID and returns the corresponding token definition object
	...
	//Returns an array of all role IDs that uses a provided token
