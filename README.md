...
The app is split into nine modules, each an IIFE exposing a small public
...
| **`ttsmanifest.js`** | Maps narration text to pre-recorded clip names. A flat lookup (`lookup`) handles whole-sentence matches; a template/hole system plus a minimum-splice-cost search (`lookupParts`) handles spliced-together clips when no whole-sentence recording exists. Populated at init from a hand-authored manifest plus generated number/role/team entries. Knows nothing about how a clip is actually played back. |
| **`audioatlas.js`** | Plays pre-recorded clips via the Web Audio API. All of a language's clips are packed into one or more shared audio files ("atlases"), decoded once and cached; each clip is a `{start, end}` region within its atlas, with independently tunable silence padding before/after playback. The atlas file and its region table are generated by an external packing script, not by this module. |
| **`autonarrator.js`** | The automatic narration and input-handling engine. Takes raw turns from the generated prompt, resolves each turn through `interpreter.js` immediately before playback, and walks the resulting text, pause, and input segments asynchronously. For each text segment, sources audio with this priority: a whole-sentence clip (`ttsmanifest.js`/`audioatlas.js`), then spliced clips, then synthesized speech as the final fallback. Owns the browser Web Speech API boundary, voice selection, pause/resume/stop state, inter-turn pacing, and interactive input waits; reports playback progress to the GUI through callbacks. Contains no presentation/UI code. |
...
5. **Render the prompt** (manual/text mode): `Interpreter.renderAll(turns,
   mode)` resolves each turn's data against its localization template,
   producing the final narration text per turn — the full "verbose"
   flavor-text script by default, or a bare-bones turn-order/outcome
   reminder for experienced narrators if brief mode is enabled.
6. The GUI stores both the raw turns and their rendered text in `promptState`.
   ...
   single-turn-navigable), while the raw form remains available to
   `AutoNarrator` for narration.  The new role selection is persisted.
...
2. The speech overlay opens, and `AutoNarrator.play(rawTurns, ...)` takes
   over: each turn is resolved through `Interpreter.resolveTurn(turn,
   "automatic", boundValues)` immediately before it plays, not batched
   upfront — so a value an earlier turn's player-input segment bound is
   already visible to every turn resolved after it. Automatic mode always
   renders the full "verbose" text — brief mode only applies to the
   written prompt.
3. `AutoNarrator` walks each turn's resulting text/pause/input segments.
   For each text segment it tries a whole-sentence pre-recorded clip
   first (`TTSManifest.lookup`, played via `AudioAtlas`), then spliced
   clips (`TTSManifest.lookupParts`) if no whole-sentence recording
   exists, and only falls back to synthesized speech if neither is
   available. Progress is reported back to `gui.js` via callbacks
   (`clbkSpeech*`), which update the overlay's spoken text, countdown,
   and — for an input segment — a set of tappable choice buttons.
...
- **Mapping narration text to a pre-recorded clip name** →
  `ttsmanifest.js`.
- **Pre-recorded clip playback (atlas decoding, splicing, silence
  tuning)** → `audioatlas.js`.
- **Spoken narration playback/pacing/voice selection, and the
  clip-vs-synthesis fallback chain** → `autonarrator.js`.
