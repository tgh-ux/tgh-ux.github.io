 ...
 *
 * Builds and maintains the application's DOM, coordinates interaction
 * between the user and the underlying modules, and reflects application
 * state visually.
 *
 * Business logic remains in Roles, Settings, Rules, Interpreter and
 * Localization; this module is primarily responsible for presentation
 * and interaction.
 */
...
//Called once after the tag filter tiles have been built (buildSelectionTagFilters), since it looks up existing .selection-filter-option elements to apply state to
...
function buildPromptNavigationControls() {
	...
	/*
	 * Settings are generated entirely from Settings.js metadata rather than
	 * being individually hard-coded into the UI.
	 */
...
/*
 * Synchronizes all UI derived from the current role selection.
 *
 * Resolves invalid selections, updates visual state, revalidates settings
 * and regenerates the narration prompt.
 */
...
/*
 * Generates the narration prompt from the current configuration.
 *
 * Settings validation occurs first. Successful generation passes through
 * Rules → Interpreter → Localization before being displayed.
 */
...
function resizePromptBox() {
	...
	/*
	 * Updates the "X%" label shown next to each weight input in a group to
	 * reflect its live share of the group's total, purely as visual feedback
	 * while editing (e.g. so a user can see "this option is currently ~30%
	 * of the pool" before saving). This is a display-only echo of the same
	 * weight-group concept Settings/Rules use for random selection — it does
	 * not affect validation (Settings.validate handles that separately) and
	 * only reads values already committed via Settings.setValue.
	 */
	...
	/*
	 * Collects and propagates validation errors of nodes in the settings tree.
	 * Recurses down first, gathering any errors generated by the child nodes.
	 * Then checks errors on itself, the local node, and renders them in the GUI.
	 * Finally returns all errors found in children and itself, propagating them
...
/*
 * Toggles a settings node's children open/closed. A node with children can
 * itself be a weight/percent/toggle item (e.g. a weighted category that also
 * branches into sub-options), in which case its own input control lives in
 * this same row alongside the disclosure toggle — the guard below stops a
 * click on that control (or the error indicator button) from also
 * collapsing/expanding the row. Child rows live in a separate `childrenWrap`
 * sibling, not inside this row, so they can't trigger this listener anyway.
 */
